...
| NFR-007 | GitHub Actions shall validate changes and produce reproducible build/test evidence. | PR CI restores locked dependencies, checks formatting/lint, builds, runs tests, checks migrations, builds all images, and uploads test/coverage artifacts. Small performance smoke tests exercise instrumentation without claiming production capacity. |
