...
1. Open immutable source streams and verify stored length/checksum during the read. Apply the explicit per-file source encoding: strict UTF-8 by default, or bounded streaming transcoding from Windows-1252, UTF-16 little-endian, or UTF-16 big-endian. Configure the selected delimiter, required headers, quotes, and multiline records. Enforce maximum columns, bytes per field/record, and decoded buffer size as parsing proceeds. Never silently fall back from one encoding to another.
2. Validate matching header sets; map columns by exact header name, not position. Remove a decoded Unicode BOM only at the beginning of a file. A leading UTF-8 byte `0xEF` that is not the complete BOM sequence remains input data and is validated normally. Validate blank records rather than skipping them; a final record terminator does not create an extra record. Reject structural problems with stable codes such as `InvalidUtf8`, `InvalidEncoding`, `MalformedRecord`, `SchemaMismatch`, `EmptyKey`, and `RecordLimitExceeded`.
